{{-- Products Section --}}
<section class="products" id="products" aria-labelledby="productsHeading">
    <div class="container">

        <div class="section-header">
            <span class="section-label">Durable &amp; Efficient</span>
            <h2 id="productsHeading">Featured <em>Products</em></h2>
        </div>

        <div class="products-grid">

            {{-- Product 1 --}}
            <article class="product-card" id="productHowoV7X" aria-label="Howo V7-X Dump Truck">
                <div class="product-img">
                    <img src="{{ asset('assets/images/truck1.png') }}" alt="Howo V7-X Dump Truck" loading="lazy">
                </div>
                <div class="product-info">
                    <div class="product-colors" aria-label="Available colors">
                        <span class="color-swatch active" style="background:#ffffff;" title="White" aria-label="White"></span>
                        <span class="color-swatch" style="background:#D94030;" title="Red" aria-label="Red"></span>
                        <span class="color-swatch" style="background:#1E2460;" title="Navy Blue" aria-label="Navy Blue"></span>
                    </div>
                    <div class="product-actions">
                        <a href="#quote" class="btn btn-red btn-sm" aria-label="Quick View Howo V7-X">Quick View</a>
                        <a href="#" class="btn btn-outline btn-sm" aria-label="Compare Howo V7-X">Compare</a>
                    </div>
                    <h3 class="product-name">Howo V7-X Dump Truck</h3>
                </div>
            </article>

            {{-- Product 2 --}}
            <article class="product-card" id="productHowoE7" aria-label="Howo E7 Dump Truck">
                <div class="product-img">
                    <img src="{{ asset('assets/images/truck2.png') }}" alt="Howo E7 Dump Truck" loading="lazy">
                </div>
                <div class="product-info">
                    <div class="product-colors" aria-label="Available colors">
                        <span class="color-swatch active" style="background:#ffffff;" title="White" aria-label="White"></span>
                        <span class="color-swatch" style="background:#D94030;" title="Red" aria-label="Red"></span>
                        <span class="color-swatch" style="background:#1E2460;" title="Navy Blue" aria-label="Navy Blue"></span>
                    </div>
                    <div class="product-actions">
                        <a href="#quote" class="btn btn-red btn-sm" aria-label="Quick View Howo E7">Quick View</a>
                        <a href="#" class="btn btn-outline btn-sm" aria-label="Compare Howo E7">Compare</a>
                    </div>
                    <h3 class="product-name">Howo E7 Dump Truck</h3>
                </div>
            </article>

            {{-- Product 3 --}}
            <article class="product-card" id="productHowoTX" aria-label="Howo TX Dump Truck">
                <div class="product-img">
                    <img src="{{ asset('assets/images/truck3.png') }}" alt="Howo TX Dump Truck" loading="lazy">
                </div>
                <div class="product-info">
                    <div class="product-colors" aria-label="Available colors">
                        <span class="color-swatch active" style="background:#ffffff;" title="White" aria-label="White"></span>
                        <span class="color-swatch" style="background:#D94030;" title="Red" aria-label="Red"></span>
                        <span class="color-swatch" style="background:#1E2460;" title="Navy Blue" aria-label="Navy Blue"></span>
                    </div>
                    <div class="product-actions">
                        <a href="#quote" class="btn btn-red btn-sm" aria-label="Quick View Howo TX">Quick View</a>
                        <a href="#" class="btn btn-outline btn-sm" aria-label="Compare Howo TX">Compare</a>
                    </div>
                    <h3 class="product-name">Howo TX Dump Truck</h3>
                </div>
            </article>

            {{-- Product 4 --}}
            <article class="product-card" id="productHowoA7Prime" aria-label="Howo A7|T7 Prime Mover">
                <div class="product-img">
                    <img src="{{ asset('assets/images/truck4.png') }}" alt="Howo A7|T7 Prime Mover" loading="lazy">
                </div>
                <div class="product-info">
                    <div class="product-colors" aria-label="Available colors">
                        <span class="color-swatch active" style="background:#ffffff;" title="White" aria-label="White"></span>
                        <span class="color-swatch" style="background:#D94030;" title="Red" aria-label="Red"></span>
                        <span class="color-swatch" style="background:#1E2460;" title="Navy Blue" aria-label="Navy Blue"></span>
                    </div>
                    <div class="product-actions">
                        <a href="#quote" class="btn btn-red btn-sm" aria-label="Quick View Howo A7 Prime Mover">Quick View</a>
                        <a href="#" class="btn btn-outline btn-sm" aria-label="Compare Howo A7 Prime Mover">Compare</a>
                    </div>
                    <h3 class="product-name">Howo A7|T7 Prime Mover</h3>
                </div>
            </article>

            {{-- Product 5 --}}
            <article class="product-card" id="productHowo7Prime" aria-label="Howo 7 Prime Truck">
                <div class="product-img">
                    <img src="{{ asset('assets/images/truck5.png') }}" alt="Howo 7 Prime Truck" loading="lazy">
                </div>
                <div class="product-info">
                    <div class="product-colors" aria-label="Available colors">
                        <span class="color-swatch active" style="background:#ffffff;" title="White" aria-label="White"></span>
                        <span class="color-swatch" style="background:#D94030;" title="Red" aria-label="Red"></span>
                        <span class="color-swatch" style="background:#1E2460;" title="Navy Blue" aria-label="Navy Blue"></span>
                    </div>
                    <div class="product-actions">
                        <a href="#quote" class="btn btn-red btn-sm" aria-label="Quick View Howo 7 Prime">Quick View</a>
                        <a href="#" class="btn btn-outline btn-sm" aria-label="Compare Howo 7 Prime">Compare</a>
                    </div>
                    <h3 class="product-name">Howo 7 Prime Truck</h3>
                </div>
            </article>

            {{-- Product 6 --}}
            <article class="product-card" id="productHowoA7Dump" aria-label="Howo A7|T7 Dump Truck">
                <div class="product-img">
                    <img src="{{ asset('assets/images/truck6.png') }}" alt="Howo A7|T7 Dump Truck" loading="lazy">
                </div>
                <div class="product-info">
                    <div class="product-colors" aria-label="Available colors">
                        <span class="color-swatch active" style="background:#ffffff;" title="White" aria-label="White"></span>
                        <span class="color-swatch" style="background:#D94030;" title="Red" aria-label="Red"></span>
                        <span class="color-swatch" style="background:#1E2460;" title="Navy Blue" aria-label="Navy Blue"></span>
                    </div>
                    <div class="product-actions">
                        <a href="#quote" class="btn btn-red btn-sm" aria-label="Quick View Howo A7 Dump">Quick View</a>
                        <a href="#" class="btn btn-outline btn-sm" aria-label="Compare Howo A7 Dump">Compare</a>
                    </div>
                    <h3 class="product-name">Howo A7|T7 Dump Truck</h3>
                </div>
            </article>

        </div>
    </div>
</section>
